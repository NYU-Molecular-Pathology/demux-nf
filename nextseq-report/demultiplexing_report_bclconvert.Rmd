...
if (project_name == "" || is.na(project_name)) {
  ss <- read.csv(file.path(reports_dir, "SampleSheet.csv"), check.names = FALSE)
  hit <- ss[grepl("^RunName$", ss[,1]), , drop = FALSE]
  project_name <- if (nrow(hit)) as.character(hit[1, 2]) else "Unknown"
}
